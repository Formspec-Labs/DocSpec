...
regulations.gov dockets, and similar public-sector sources — into one verified,
immutable shared `derivation` artifact that other software can trust without
trusting this repository. A `DocumentRelease` is DocSpec's semantic state inside
that artifact, not a second exchange format.

The idea in four steps:

1. **Consume** — a sealed shared `source-catalog` artifact names exactly which
   source files exist, pinned by digest. DocSpec never decides what to fetch; it
...
3. **Publish** — everything is sealed into one portable, immutable shared
   `derivation`: closed membership, every member digest-pinned, coverage
   accounted in both directions (every source claim lands somewhere; every
   published claim traces to a source).
...
The seam is shared `source-catalog → derivation`. DocSpec reads and writes its
`DocumentRelease` semantic state through that one artifact protocol. Ownership
rows and the exchange rule live in REF-024 (`RefSpec/docs/decisions.md`) —
products trade published, digest-pinned files, never source trees.
...
needs the published shared derivation and the installed `docspec` package —
