...
`examples/phrase_match_processor.py` implements the existing `Processor` interface
...
The constructor accepts an existing `ProcessorResourceIdentity` of kind
`REFERENCE_DATA`, exact JSON bytes, optional `case_sensitive`, processor item limits
and a retry policy. It verifies the resource's SHA-256 before parsing and stores
...
digest and are defined in this guide and the example source. The description
exposes ordinary item limits and the reference-data identity directly.
...
Each UTF-8 text segment produces one result record. `matches: []` records a
...
existing `ProcessorDescription` identity. Existing planning then reuses compatible
