...
The Core model and ordered implementation tasks now govern the local runtime.
Earlier decisions below retain their source-policy scope and historical evidence;
retired release, store and profile APIs are not current implementation guidance.
...
| [0007: table-shaped states by reference](0007-table-shaped-states-by-reference.md) | A keyed state whose members are the rows of a producer's pinned Parquet, with occurrence identity from key and row content; typed derived layers | **Proposed, not accepted,** with open rulings R1–R6 recorded in the decision. Nothing is implemented; C27 and C29 in the [Core task list](../core-model-implementation-tasks.md) are the proposed tasks. The [D2 spike](../history/probes/2026-09-23-admit-by-reference-spike.md) is evidence for a ruling, not an implementation. |

The current [record storage](../record-storage.md) uses Parquet and DuckDB, with
SQLite as the authoritative Core metadata ledger. Current regression and capacity
status belongs in the [Core task list](../core-model-implementation-tasks.md),
not in an inherited test count from a previous runtime.
