...
generated wiki pages do not override accepted rules or current executable checks.
...
| --- | --- | --- |
| [0001: DocumentRelease 2.0](0001-document-release-2-0.md) | Portable bundle shape, digest preimages, evidence, retention floors, attachments/comments; later amendments supersede earlier identity tables | The portable builder and verifier use the current eight-schema shape. Predecessor reader support ended on 2026-09-11; frozen fixtures retain provenance. This decision supersedes the older specification's `1.1` publication shape and `documentStateDigest` definition for this format. The application state model is separately implemented and also advertises `2.0`; see the architecture guide before choosing a verifier. Local mint receipts are historical local evidence. |
| [0002: execution and acquisition gaps](0002-shared-execution-and-the-acquisition-gap-ledger.md) | Retry classification, complete accounting of acquisition gaps, and the division between source acquisition and document processing | DocSpec has bounded execution, retries, and checkpoint recovery. Upstream acquisition-ledger completion is not established by those tests. Apply the later retry ruling and 0006 when assessing source-native publication; this index does not claim upstream implementation or release. |
| [0003: Federal Register identity](0003-federal-register-record-identity.md) | Upstream composite record identity and recorded collapse; source text stays unchanged | The decision records a September 5 rebuild and its acceptance evidence. DocSpec resolves installed producer profiles through `adapters/spicyregs_source_native.py`; a checkout does not establish which producer package or external release a deployment uses. Verify the installed reader and artifact pins for a campaign. |
