...
memberCount                 207
totalMemberByteSize     813,363,843
...
| **`documentStateDigest` is blind to per-row payload fields.** | Measured here, not theorised: correcting 1,683 Mirrulations `sourceMetadata.sourceUrl` values from the metadata sibling to the carried body moved `totalMemberByteSize` by 11,781 bytes and moved the `globalManifest` digest, and left `documentStateDigest` and `releaseId` exactly where they were. The preimage is the root's `content`, which carries digests, counts, and byte totals; a row field that no set digest names and no byte total sums — `sourceMetadata`, an excluded range's prose `reason`, a segment's `headingPath` — changes the bundle without renaming it. That follows from the sealed rule (Decision 0001, identity rule 1) rather than departing from it, and the same rule is what makes a physical repack identity-preserving. It is recorded because a reader who takes `documentStateDigest` to mean "is this the same corpus, in every field" will be wrong. |
