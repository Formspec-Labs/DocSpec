...
must not import ``docspec.corpora`` at runtime, and a separate test proves
that.
"""
...
from docspec.corpora import relation_exclusion_evaluation_v2 as old_v2
from docspec.docpipeline.adapters import SHARED_CALL_DETAIL_KEYS, StructuredTextResult
...
from docspec.docpipeline.relation_task import (
    RelationV2Task,
    load_answers,
    load_corpus,
    units_from_corpus,
)
from docspec.docpipeline.runtime import RunPlan, validate_run
from docspec.ontology.common import canonical_json
...
def test_the_new_step_never_imports_the_old_runner() -> None:
    for module in ("extraction", "relation_task"):
        source = (Path(__file__).parents[1] / "src" / "docspec" / "docpipeline" / f"{module}.py").read_text(
            encoding="utf-8"
        )
        assert "docspec.corpora" not in source
        assert "docspec.ontology.llm" not in source


def test_the_old_runner_and_its_command_are_untouched() -> None:
    assert (
        Path(__file__).parents[1] / "src" / "docspec" / "corpora" / "relation_exclusion_evaluation_v2.py"
    ).is_file()
