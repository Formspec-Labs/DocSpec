...
from docspec.corpora import segmentation_experiment as legacy_dense
from docspec.corpora import segmentation_rerank as legacy_rerank
from docspec.corpora import segmentation_sparse_retrieval as legacy_sparse
from docspec.docpipeline import retrieval as v3_retrieval
...
def test_production_docpipeline_has_no_legacy_identity_or_corpora_import() -> None:
    root = Path(__file__).parents[1] / "src" / "docspec" / "docpipeline"
...
def test_legacy_runners_and_commands_remain_present() -> None:
    ...
    assert all((root / "src" / "docspec" / "corpora" / module).is_file() for module in modules)
