...
from docspec.corpora.segmentation_experiment import (
    ExperimentConfig,
...
from docspec.docpipeline.source import build_source_artifacts
from docspec.ontology.common import canonical_json, text_digest, write_parquet_rows
from docspec.ontology.subjects import build_artifacts as legacy_artifacts
...
def test_new_production_modules_do_not_import_old_runners() -> None:
    ...
    production = (root / "src" / "docspec" / "docpipeline" / "segments.py").read_text(encoding="utf-8")

    assert "docspec.corpora" not in production
    assert "docspec.ontology.segmentation" not in production
    assert "docspec.ontology.subjects" not in production
    assert (root / "src" / "docspec" / "corpora" / "segmentation_experiment.py").is_file()
