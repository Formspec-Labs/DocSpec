...
from docspec.docpipeline.adapters.sentence_transformers import DenseEmbeddingResult
...
from docspec.docpipeline.runtime import PlanError
...
def test_dense_ranking_l2_normalizes_uses_dot_and_breaks_ties_by_target() -> None:
    ...
    module = __import__("docspec.docpipeline.retrieval", fromlist=["x"])
...
def test_dense_import_boundaries_and_legacy_runners_remain_untouched() -> None:
    ...
    assert not any(name == "docspec.corpora" or name.startswith("docspec.corpora.") for name in imports)
    assert not {"sentence_transformers", "torch", "tiktoken", "openai"} & imports
    private_step_imports = {
        alias.name
        for node in ast.walk(tree)
        if isinstance(node, ast.ImportFrom)
        and node.module in {"docspec.docpipeline.source", "docspec.docpipeline.segments"}
        for alias in node.names
        if alias.name.startswith("_")
    }
    assert private_step_imports == set()
    assert "incumbent-three-table-whole-row-v1" not in source
    assert Path("src/docspec/corpora/segmentation_experiment.py").is_file()
    assert Path("src/docspec/corpora/artifact_retrieval_baseline.py").is_file()
