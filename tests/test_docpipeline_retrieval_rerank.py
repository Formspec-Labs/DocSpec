...
from docspec.docpipeline.adapters.sentence_transformers import RerankResult
...
from docspec.docpipeline.runtime import PlanError, WorkCheckpoint
...
def test_rerank_import_guards_score_confinement_and_legacy_runner_preservation() -> None:
    ...
    extraction = Path("src/docspec/docpipeline/extraction.py").read_text(encoding="utf-8")
    assert "refuse_retrieval_aids(payload)" in extraction
    assert '"rerank_score"' in extraction
    assert Path("src/docspec/corpora/segmentation_rerank.py").is_file()
