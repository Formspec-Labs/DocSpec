...
from docspec.transforms.build_regulatory_agenda import (
    ITEM_COLUMNS,
...
def test_agenda_items_classify_scope_without_using_rin_as_action_identity(
    ...
    items = {
        row["rin"]: row for row in pq.read_table(items_file).to_pylist()
    }
    ...
    assert items[recurring_rin]["agenda_item_id"] == (
        f"urn:rkaf:us:rin:{recurring_rin}"
    )
    ...
    assert items[ua_only_rin]["scope_basis"] == (
        "zero_evidence_linked_proceedings"
    )
    assert {
        row["proceeding_id"]
        for row in relationships
        if row["rin"] == recurring_rin
    } == set(proceedings[:2])
    ...
    assert all(
        row["relationship_role"] == "agenda_tracks_proceeding"
        for row in relationships
    )
