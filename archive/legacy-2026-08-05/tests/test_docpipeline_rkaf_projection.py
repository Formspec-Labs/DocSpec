...
from docspec.docpipeline.adapters import StructuredTextResult
...
from docspec.candidate_release import VocabularyAtlasCandidateSource
from refspec import (
    ConceptLabel,
    ConceptRelation,
    ReferenceRuntimeStore,
)
from docspec.ontology.attestations import DECISION_APPROVED, DECISIONS
...
def test_the_contract_findings_are_configuration_not_code(corpus: Path, tables: Path, monkeypatch: Any) -> None:
    ...
    """
    from docspec.docpipeline import rkaf_projection
...
def test_the_cli_reaches_both_provider_arms_without_a_credential(monkeypatch: Any) -> None:
    ...
    from docspec.docpipeline.adapters.anthropic import ProviderConfigurationError
...
def test_the_drawn_holdout_stays_sealed() -> None:
    ...
    source = (
        Path(__file__).resolve().parents[1] / "src" / "docspec" / "docpipeline" / "rkaf_projection.py"
    ).read_text()
