...
from docspec.ontology.common import RunContext
from docspec.pipelines.materialized import DatasetStage, MaterializedDatasetPipeline
from docspec.adapters.spicyregs_store import document_store as r2
...
def test_materialized_publication_commits_pointer_last(tmp_path, monkeypatch) -> None:
    ...
    def record_upload(path, *, object_key, allow_shrink=False):
        assert path.exists()
        uploads.append((object_key, allow_shrink))

    monkeypatch.setenv("R2_ACCESS_KEY_ID", "test-key")
    monkeypatch.setenv("R2_SECRET_ACCESS_KEY", "test-secret")
    monkeypatch.setenv("R2_ENDPOINT", "https://test.r2.example")
    monkeypatch.setattr(r2, "publish", record_upload)
    PublishedDataset(
        output_dir=tmp_path,
        skip_upload=False,
        run_id="publish-test",
...
def test_internal_artifact_is_generation_bound_but_not_public(
    ...
    monkeypatch.setattr(
        r2,
        "publish",
        lambda path, *, object_key, allow_shrink=False: (
            uploads.append(str(object_key))
        ),
    )
...
def test_materialized_publication_validates_before_first_upload(
    ...
    monkeypatch.setattr(
        r2,
        "publish",
        lambda path, *, object_key, allow_shrink=False: uploads.append(path),
    )
...
def test_publishing_refreshes_existing_local_source(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(r2, "fetch", download)
...
def test_dry_run_may_reuse_existing_local_source(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(r2, "fetch", unexpected_download)
...
def test_publishing_discards_cached_prior_when_remote_has_none(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(r2, "fetch", lambda remote_key, local_path: False)
    manifest = StatefulDataset(
        output_dir=tmp_path,
        skip_upload=False,
