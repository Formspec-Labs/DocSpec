...
it launches ``python -m docspec.docpipeline.adapters.docling`` in its own
...
def test_the_default_command_launches_the_adapter_module_in_isolated_mode(tmp_path: Path) -> None:
    ...
    assert WORKER_MODULE == "docspec.docpipeline.adapters.docling"
...
def _fake_call(parser_call_type: Any, offsets_type: Any) -> Any:
    ...
    from docspec.docpipeline.adapters import docling as adapter
...
def test_the_worker_reports_an_absent_extra_in_one_word(tmp_path: Path) -> None:
    from docspec.docpipeline.adapters.docling import DoclingUnavailableError, run_worker
...
def test_the_worker_records_a_parse_failure_without_repeating_its_message(tmp_path: Path) -> None:
    from docspec.docpipeline.adapters.docling import DoclingParseError, OffsetSemantics, ParserCall, run_worker
...
def test_an_unreadable_job_file_exits_non_zero_and_writes_nothing(tmp_path: Path) -> None:
    from docspec.docpipeline.adapters.docling import run_worker
...
def test_the_worker_entry_takes_exactly_one_argument() -> None:
    from docspec.docpipeline.adapters.docling import main
