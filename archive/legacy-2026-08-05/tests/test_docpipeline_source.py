...
from docspec.docpipeline import source as source_module
from docspec.docpipeline.runtime import CheckResult, scan_text_for_secrets
...
from docspec.ontology.common import write_parquet_rows
...
SOURCE_PATH = REPOSITORY_ROOT / "src" / "docspec" / "docpipeline" / "source.py"

FORBIDDEN_SOURCE_IMPORTS = (
    "docspec.corpora",
    "docspec.ontology.subjects",
    "docspec.ontology.adapters",
    "docspec.ontology.segmentation",
    "docspec.docpipeline.segments",
    "docspec.docpipeline.retrieval",
    "docspec.docpipeline.extraction",
    "docspec.docpipeline.workflow",
    "docspec.docpipeline.cli",
    "docling",
    "docling_core",
    "duckdb",
    "openai",
...
def test_source_imports_no_replaced_runner_and_no_provider_package() -> None:
    ...
    assert "docspec.docpipeline.adapters.docling" in modules
...
def docling_uninstalled(monkeypatch: pytest.MonkeyPatch) -> Iterator[None]:
    ...
    monkeypatch.delitem(sys.modules, "docspec.docpipeline.adapters.docling", raising=False)
...
def test_source_imports_and_parses_native_structure_without_docling_installed(
    ...
    monkeypatch.delitem(sys.modules, "docspec.docpipeline.source", raising=False)
    reloaded = import_module("docspec.docpipeline.source")
    reload(reloaded)
