...
from docspec.ontology.rulespec_release import require_released_rulespec
from docspec.pipelines.materialized import DatasetStage, MaterializedDatasetPipeline
from docspec.transforms import (
...
class OntologyDatasetPipeline(MaterializedDatasetPipeline):
    ...
    def __init__(
        ...
        *,
        ...
        rulespec_declaration: Path | None = None,
        **kwargs,
    ) -> None:
        super().__init__(**kwargs)
        self.full_refresh = full_refresh
        self.rulespec_declaration = rulespec_declaration or (Path.cwd() / "conformance" / "rulespec-l0.yaml")
        ...
        for output_name, prior_name in (
            ...
            if not prior.exists():
                if output_name == "ontology_segment_ledger.parquet":
                    from docspec.ontology.ledger import (
    ...
    def validate_before_publish(self, manifest_path: Path) -> None:
        ...
        from docspec.ontology.receipt import validate_generation
...
def main(
    *,
    ...
    rulespec_declaration: Annotated[
        Path | None,
        Parameter(help="Rulespec L0 declaration used to authorize publication"),
    ] = None,
) -> None:
    OntologyDatasetPipeline(
        ...
        rulespec_declaration=rulespec_declaration,
    ).run()
