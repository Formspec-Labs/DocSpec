...
Ordinary :class:`~docspec.pipelines.rollups.base.RollupPipeline` jobs publish
...
from docspec.adapters.spicyregs import Pipeline
from docspec.adapters.spicyregs_store import document_store
from docspec.ontology.common import RunContext, canonical_json, stable_id
...
class MaterializedDatasetPipeline(Pipeline):
    ...
    def _prime_sources(self, output_dir: Path) -> None:
        ...
        for remote_key in self.source_inputs:
            ...
            if self.skip_upload and local.exists():
                ...
                continue
            if not self.store.fetch(remote_key, local):
                raise RuntimeError(
    ...
    def _prime_previous_generation(self, output_dir: Path) -> dict | None:
        ...
        if not use_cached_pointer and not self.store.fetch(pointer_key, pointer_path):
        ...
        if not use_cached_manifest and not self.store.fetch(manifest_key, manifest_path):
        ...
        for artifact_name, prior_name in prior_declarations:
            ...
            if not use_cached_artifact and not self.store.fetch(remote_key, target):
    ...
    def _publish(
        ...
        *,
        ...
        for name, path in artifact_paths.items():
            ...
            self.store.publish(path, object_key=artifacts[name]["remote_key"])

        pointer = _read_json(pointer_path)
        manifest_key = str(pointer["manifest_key"])
        self.store.publish(manifest_path, object_key=manifest_key)
        ...
        self.store.publish(
            pointer_path,
            object_key=f"{_ROOT_PREFIX}/{self.dataset_name}/latest.json",
            allow_shrink=True,
        )
        logger.info(
