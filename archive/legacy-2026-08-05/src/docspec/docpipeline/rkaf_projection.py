...
come from :mod:`docspec.docpipeline.source` (never re-derived here). Fragment
...
:mod:`docspec.ontology.citations`. Relationship assertions are re-serialized
...
tested concept-assignment path (:class:`~docspec.docpipeline.tag_task.TagExtractionTask`
driven by :func:`~docspec.docpipeline.extraction.run_extraction`), which
...
from docspec.candidate_release import (
    CandidateConceptBridge,
    CandidateReleaseSource,
)

from docspec.docpipeline.source import (
    SourceArtifact,
    build_source_artifact,
    profile_for_table,
)
from docspec.ontology.attestations import (
    ATTESTOR_KIND_AI_MODEL,
    DECISION_ENDORSED_FOR_REVIEW,
    attestation_row,
)
from docspec.ontology.citations import (
    canonical_cfr_iri,
    canonical_pl_iri,
...
from docspec.ontology.common import RunContext, canonical_json, stable_id, text_digest
from docspec.ontology.llm import resolve_exact_evidence_offsets
...
class PublishedTables:
    ...
    Reads go through :func:`~docspec.ontology.common.read_parquet_rows`
    rather than a query engine: ``docspec.docpipeline`` keeps direct DuckDB
    ...
    def rows(self, table: str, **equals: Any) -> list[dict[str, Any]]:
        ...
        from docspec.ontology.common import read_parquet_rows
...
def load_artifact(
    ...
    *,
    ...
    Records come from :func:`~docspec.docpipeline.source.iter_source_records`,
    ...
    :func:`~docspec.docpipeline.source.build_source_artifact`. This function
    only finds the record and hands it over.
    """
    from docspec.docpipeline.source import iter_source_records
...
def _source_table_for_profile(profile_id: str) -> str:
    from docspec.docpipeline.source import SOURCE_PROFILES
...
def _run_model_layer_with_vocabulary(
    ...
    *,
    ...
    """
    from docspec.docpipeline.extraction import (
        extraction_plan_facts,
        plan_extraction_items,
        run_extraction,
    )
    from docspec.docpipeline.runtime import RunPlan
    from docspec.docpipeline.segments import SegmentSettings, segment_artifact
    from docspec.docpipeline.tag_task import TagExtractionTask, tag_unit
    from docspec.enrichment.connected_concepts import (
        CONNECTED_INDEXED_REPRESENTATION_VERSION,
        CONNECTED_SELECTOR_VERSION,
        select_connected_candidate_concepts,
    )
    from docspec.ontology.concepts import (
        ANCHORED_SELECTOR_VERSION,
        select_candidate_concepts_anchored_v2,
    )
    from docspec.ontology.segmentation import TiktokenCounter
