...
:class:`~docspec.document_release.StructuralPassage` records over one exact
:class:`~docspec.document_release.TextRepresentation`, while
:mod:`docspec.docpipeline.segments` consumes a
:class:`~docspec.docpipeline.source.SourceArtifact` and the processing-region
stream :func:`~docspec.docpipeline.source.processing_regions` derives from it.
...
* **One sealed passage is one processing region.** Passage coordinates are
  ...
  :mod:`~docspec.docpipeline.segments` writes down — so the offsets pass
...
* **Text outside every sealed passage is an ineligible region, not a gap.** The
  ...
  :func:`~docspec.docpipeline.segments.segment_artifact` records in
...
from docspec.docpipeline.runtime import scan_text_for_secrets
from docspec.docpipeline.segments import (
    SegmentOutcome,
    SegmentSettings,
    TokenCounter,
    segment_artifact,
...
from docspec.document_release import (
...
class ReleaseValidationError(DocumentReleaseSegmentError):
    ...
    Raised when :func:`~docspec.document_release.validate_document_release`
...
def segment_document_release(
    ...
    *,
    ...
    go straight to :func:`~docspec.docpipeline.segments.segment_artifact`.
    """
    results = tuple(
...
def check_release_reversibility(
    ...
    Stronger than :func:`~docspec.docpipeline.segments.check_segment_slices`,
